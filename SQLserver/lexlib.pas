...
uses uStmt; {for GlobalParseStmt} 
...
var
//{$LONGSTRINGS ON}
GlobalParseStmt:TStmt;              //pass current transaction to parse routine for access to SQL buffer
                                    //this needs to span protected calls to the parser...
yyOutputText:string;             {output buffer}
//{$LONGSTRINGS OFF}
...
//{$LONGSTRINGS OFF}
yylineno, yycolno, yycolnolastline : Integer;     (* current input position *)
yyOffset: integer; (* offset in buffer *)
yytext            : String;      (* matched text (should be considered r/o) *)
{***yyleng            : Byte         (* length of matched text *)
  absolute yytext;  (* note : this is not a good idea with long strings! *)
}
...
const nl = #10;  (* newline character *)
      cr = #13;  { carriage return (Greg Gaughan 25/03/01) }  


function get_char : Char;
  begin
  if GlobalParseStmt.InputText<>'' then
  begin
    if GlobalParseStmt.InputText[1]=nl then begin inc(yylineno); yycolnolastline:=yycolno; yycolno:=0; end;
    get_char:=GlobalParseStmt.InputText[1];
    delete(GlobalParseStmt.InputText,1,1);  //note: poor speed?
    inc(yycolno);
    inc(yyOffset); //custom
  end
  else
...
procedure unget_char ( c : Char );
  begin
    insert(c,GlobalParseStmt.InputText,1); //note: check versioning is not too harsh
    if c=nl then begin dec(yylineno); yycolno:=yycolnolastline+1; end;
    dec(yycolno);
    dec(yyOffset); //custom
  end(*unget_char*);
...
procedure put_char ( c : Char );
  begin
    if c=#0 then
      ...
      yyOutputText:=yyOutputText+#13  //writeln(yyoutput)
    else
      yyOutputText:=yyOutputText+c    //write(yyoutput, c)
  end(*put_char*);
...
max_matches = 2048; //21/01/03 Greg Gaughan 1024;
max_rules   = 512;  //21/01/03 Greg Gaughan 256;
...
procedure echo;
  var i : Integer;
  begin
    for i := 1 to length(yytext){***yyleng} do
      put_char(yytext[i])
  end(*echo*);
...
procedure yyless ( n : Integer );
  var i : Integer;
  begin
    for i := length(yytext){***yyleng} downto n+1 do
      unget_char(yytext[i]);
    setLength(yytext,{***yyleng := }n);
  end(*yyless*);
...
procedure reject;
  ...
  begin
    yyreject := true;
    for i := length(yytext){***yyleng}+1 to yysleng do
...
function yywrap : Boolean;
  begin
    //closeFile(yyinput); closeFile(yyoutput);
    yywrap := true;
  end(*yywrap*);
...
procedure yynew;
  begin
    if yylastchar<>#0 then
      if (yylastchar=nl) (*todo reinstate? or (yylastchar=cr){Greg Gaughan 25/03/01}*) then
        yylstate := 1
      else
        yylstate := 0;
...
procedure yyscan;
  begin
    ...
    yyactchar := get_char;
    (****
    {setlength(yytext,length(yytext)+1)}{***}inc(yyleng){};
    yytext[{length(yytext)}{***}yyleng{}] := yyactchar;
    *)
...
function yyfind ( var n : Integer ) : Boolean;
  begin
    ...
    if yymatches>0 then
      begin
        yysleng := length(yytext){***yyleng};
        n       := yystack[yymatches];
        yyless(yypos[n]);
        yypos[n] := 0;
        if length(yytext){***yyleng}>0 then
          yylastchar := yytext[length(yytext){***yyleng}]
        else
          yylastchar := #0;
        yyfind := true;
      end
...
begin
//  assignFile(yyinput, 'test2.sql');
//  assignFile(yyoutput, 'test2.prs');
